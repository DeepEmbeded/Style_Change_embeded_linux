...
} rknn_whisper_subgraph_t;

typedef struct
{
    rknn_whisper_subgraph_t encoder_context;
    rknn_whisper_subgraph_t decoder_context;
} rknn_whisper_context_t;

int init_whisper_model(const char *model_path, rknn_whisper_subgraph_t *app_ctx, int npu_index);


int release_whisper_model(rknn_whisper_subgraph_t *app_ctx);
